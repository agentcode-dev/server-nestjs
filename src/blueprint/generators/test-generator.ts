...
export class TestGenerator {
  generate(blueprint: Blueprint): string {
    ...
    push(`// Generated by AgentCode Blueprint — do not edit directly.`);
    push(`// Source: .agentcode/blueprints/${blueprint.source_file}`);
    ...
    push(`// published helper re-exported from \`@agentcode-dev/agentcode-nestjs/testing\`.`);
    push(`import { buildEnv } from '../../test/helpers/make-controller';`);
    push(`import type { AgentCodeConfig } from '@agentcode-dev/agentcode-nestjs';`);
    push(``);

    // Config helper
    push(`function makeConfig(overrides: Partial<AgentCodeConfig> = {}): AgentCodeConfig {`);
    push(`  return {`);
    push(`    models: {`);
    push(`      ${slug}: {`);
    push(`        model: '${modelKey}',`);
